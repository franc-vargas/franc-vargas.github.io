...
We'll also create a function to simulate data. This function is taken from Tidymodel's Conformal Prediction post, but we won't be using any other function from the Tidymodels framework.
...
To create our prediction intervals, we need to define a 'distance' or 'score' function for our calibration set residuals $R_i$, $i \in D_2$. We can use any negatively oriented function (where lower values are better!). I'll use the absolute difference: $R_i = |Y_i - \hat f_{n_1}(X_i)|,\ i \in D_2$ since it's easy to understand, and it maintains the scale of the original problem. 
...
So, since calibration set has 1000 data points, the rank we're looking for would be: `r ceiling((1 - 0.05) * (nrow(cal_data)+1))`, since this is an index, we'll need to sort our residuals, and look for the index. 
...
cbind(rank_value, 
      quantile_value) |> 
  knitr::kable(col.names = c("Rank method",
                             "Quantile method"),
               row.names = F,
               format = "html")
```
...
So, using our test set of 10.000 observations, we can see that the coverage is actually 95 %.

This is extremely relevant, since previously most Machine Learning methods lacked a reliable way to measure the uncertainty of their methods, and relying on asymptotic normality does not always make sense.
